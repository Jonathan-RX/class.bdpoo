...
Date : 12/2009
Dernière modification : 06/2017 -> Utilisation de Mysqli pour compatibilité avec PHP7
...
class Bd{
	...
	public function insert($table_name, $data){
		$insert_id = '';
		$insert_value = '';
		$connect = mysqli_connect($this->host, $this->login, $this->password,$this->basename);
	...
	public function get_results($request){
		$connect = mysqli_connect($this->host, $this->login, $this->password,$this->basename);
		$results = mysqli_query($connect, $request);
		$reponse = array();
		$counter = 0;
	...
	public function test_connect($host = '', $login = '', $password = '', $basename = ''){
		if(mysqli_connect($host, $login, $password, $basename)){
				return true;
		}else{
			return false;
		}
